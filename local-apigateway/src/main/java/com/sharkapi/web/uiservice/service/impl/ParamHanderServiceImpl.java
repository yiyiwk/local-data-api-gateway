...
import com.sharkapi.web.uiservice.mapper.ParamHanderMapper;
...
@Service("ParamHanderService")
public class ParamHanderServiceImpl implements IndexService {
    ...
    public List<Map> all(JSONObject json){
        return paramHanderMapper.getHandlerList(json.getInteger("interfaceId"));
    }
    ...
    public Map insert(JSONObject json) throws ResultException{
        ...
        checkName(json);
        //先查询当前接口有几个处理器
        List<Map> handlers = paramHanderMapper.getHandlerList(json.getInteger("interfaceId"));
        Map map = new HashMap();
        ...
        if(paramHanderMapper.insertHandler(map) != 1)
            throw new ResultException(143,"新增处理器失败，请稍后再试。");
        return paramHanderMapper.checkName(json.getString("name"));
    }
    ...
    public boolean checkName(JSONObject json) throws ResultException{
        ...
        Map<String, Object> name = paramHanderMapper.checkName(json.getString("name"));
    ...
    public String delete(JSONObject json) throws ResultException{
        ...
        paramHanderMapper.deleteHandler(json.getInteger("id"));
        return "删除处理器成功";
    ...
    public List<Map> sortUp(JSONObject json) throws ResultException{
        ...
        //排序必须参数 id,
        Map map = paramHanderMapper.getHandlerOne(json.getInteger("id"));
        Integer sort = Integer.valueOf(map.get("sort").toString());
        Map sortMap = new HashMap();
        sortMap.put("interfaceId",map.get("interfaceId"));
        sortMap.put("sort", sort-1);
        paramHanderMapper.updateSortUp(sortMap);
        paramHanderMapper.updateSortUpWithId(map);
        return paramHanderMapper.getHandlerList(Integer.valueOf(map.get("interfaceId").toString()));
    }
    ...
    public List<Map> sortLow(JSONObject json) throws ResultException{
        ...
        sortMap.put("interfaceId",map.get("interfaceId"));
        sortMap.put("sort", sort+1);
        paramHanderMapper.updateSortLow(sortMap);
        paramHanderMapper.updateSortLowWithId(map);
        return paramHanderMapper.getHandlerList(Integer.valueOf(map.get("interfaceId").toString()));
    }
    ...
    public Map update(JSONObject json) throws ResultException{
        ...
        if(paramHanderMapper.update(updateMap) !=1 )
            throw new ResultException(144,"处理器修改失败，请稍后重试。");
        return paramHanderMapper.getHandlerOne(json.getInteger("id"));
    }
}
